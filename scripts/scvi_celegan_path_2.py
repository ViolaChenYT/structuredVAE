...
def train_scvi_on_path_with_hvg(
    ...
    """
    Train scVI model on a lineage path subset with highly variable genes selection.
    ...
    if group_to_nodes is not None and fuzzy_mapping is not None:
        ...
        for lineage_label in lineage_path:
            ...
            if lineage_label in group_to_nodes:
                ...
                # If not found in group_to_nodes, keep the original label
                print(f"  {lineage_label} not found in group_to_nodes, keeping original")
                expanded_lineage_set.add(lineage_label)
    ...
    # Assert that all expected lineages are present (original behavior)
    assert set(adata_path.obs["lineage"].unique().tolist()) == set(expanded_lineage_path)
    
    # Apply highly variable genes selection (original approach from scvi_celegan_path_2.py)
    adata_path_copy = adata_path.copy()
    torch.set_float32_matmul_precision("high")
    sc.pp.normalize_total(adata_path_copy)
    sc.pp.log1p(adata_path_copy)
    sc.pp.highly_variable_genes(adata_path_copy, n_top_genes=2000, batch_key="batch", subset=True)
    adata_path = adata_path[:,adata_path_copy.var_names].copy()
    adata_path.layers["counts"] = adata_path.X.copy().tocsr()
    
    # Save loom file if requested
    if save_loom:
        loom_path = f"{base_path}/data/{path_name}-2.loom"
    ...
    # Get model path
    model_path = f"{base_path}/data/scvi_path_{path_name}-2/"
...
def main():
    ...
    # Use the modular function with highly variable genes approach
    result = train_scvi_on_path_with_hvg(
        adata=adata,
